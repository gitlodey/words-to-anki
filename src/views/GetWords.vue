<template>
  <div>
    ...
    <textarea
      ...
      cols="30"
      rows="10"
      v-model="state.newWordsList"
      @input="handleTextareaSearch"
    ></textarea>

    <div>
      <word-card
        v-for="item in state.wordWithMeanings"
        :key="item.word"
        :card="item"
      />
    </div>
...
const serializeLocalJsonToDictonaryApi = (
  meaning: LocalJsonWord | undefined
): DictonaryApiDefinition[] => {
  if (!meaning) {
    return [];
  }
...
const handleInputSearch = async (word: string) => {
  ...
  const meaningFromDictonaryApi = (await findMeaning(
    preparedWord
  )) as DictonaryApiResponse;
  const meaningFromLocalJson = (await findMeaningInLocalJson(
    preparedWord
  )) as LocalJsonWord;
  const serializedDefinitions =
    serializeLocalJsonToDictonaryApi(meaningFromLocalJson);
  ...
  const definitionFormattedForAnki = formatDefinitionsForAnki(
    meaningFromDictonaryApi
  );
...
const findMeaning = async (word: string) =>
  await Dictionaryapi.getMeaning(word);
const findMeaningInLocalJson = async (word: string) =>
  await LocalJsonApi.getMeaning(word);
</script>
